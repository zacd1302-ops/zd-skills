---
name: automate-me
description: Draft or revise a personal mode skill from repeated working preferences.
...
Look recursively for `.agents/skills/**/*-mode/SKILL.md` and `~/.agents/skills/*-mode/SKILL.md` matching the user's handle. Mode skills can live in a personal category directory (`.agents/skills/<handle>/`), not only at the top level. If one exists, ask the user to choose (unless they already said "update my skill" or similar):
...
Mining misses intent that hasn't come up yet. Ask the user structured, multi-choice questions rather than asking them to type from scratch. Lower cognitive load, higher hit rate.
...
Use the repository's skill-authoring guidance to author the skill. Placement:
...
- Frontmatter formatting: follow the repository's YAML rules. Keep `description` as one YAML scalar; quote it or use `description: >-` with indented continuation lines when punctuation or wrapping requires it.
...
Apply the **unslop** skill and the repository's agent-writing guidance to every line. Both apply to any agent-read prose, not just skills.
...
- User wants a task-specific skill (not working conventions): use the repository's skill-authoring guidance without mining required.
...
- The repository's skill-authoring guidance: skill authoring process and writing guidelines.
